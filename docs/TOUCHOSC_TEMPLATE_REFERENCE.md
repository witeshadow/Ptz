...
The template is provided as a JSON `.json` file, which you can import into touchOSC Editor or manually create using the step-by-step instructions in `CREATE_TOUCHOSC_TEMPLATE.md`. For direct editing, the template structure follows standard JSON format:

### Change Camera Names

In the `controls` array, find the button with id `"button_cam1"`:
...
In the `controls` array, find the fader with id `"fader_pan"` and modify the color:

```json
{
  "type": "fader",
  "id": "fader_pan",
  "color": {"r": 255, "g": 0, "b": 136},
  ...
}
```

Color values use RGB (0-255). Example: `{"r": 0, "g": 136, "b": 255}` = Blue

### Add New Camera Button

Add a new object to the `controls` array:
...
In the `controls` array, find the fader (example: `"fader_pan"`) and modify x, y, width, height in pixels.

### Change OSC Port

In the root object, modify the network section:
...
The template is provided as `touchosc-ptz-template.json`:
...
**Option 1 (Recommended):** Use touchOSC Editor (visual, no file format issues)
- See `CREATE_TOUCHOSC_TEMPLATE.md` for step-by-step instructions
- Edit controls in GUI, no need to understand file format

**Option 2:** Direct JSON editing
- Edit `touchosc-ptz-template.json` in any text editor
- Import into touchOSC Editor or app
